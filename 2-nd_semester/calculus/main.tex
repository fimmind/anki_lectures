...
\begin{note}{11c8d7341f374a2e8d7d5c8df97eaba8}
    В чём основная идея доказательства критерия строгой выпуклости (достаточность)?
...
\begin{note}{82fe965871ac446facad207a4f246b18}
    ...
    \begin{cloze}{1}
        Из теоремы о предельном переходе в неравенстве.
    \end{cloze}
\end{note}
...
\begin{note}{9390116052df401f8413ffb225259a9d}
    ...
    \begin{cloze}{1}
        Из существования односторонних производных \({ f }\) в любой точке \({ (A, B) }\).
    \end{cloze}
\end{note}
...
\begin{note}{1da04fcb23dc406eba98567735e9e6dc}
    ...
    Как показать, что прямая \( y = f(a) + k(x - a) \) является опорной для \({ f }\) \({ \implies }\) \({ k \in [f'_-(a), f'_+(a)] }\)?

    \begin{cloze}{1}
        Представить \({ f(x) - f(a) }\) как
        \[
             f'_{\pm}(a) (x - a) + o(x - a), \quad x \to a^{\pm}
        \]
        и вычесть из обоих частей равенства \({ k(x - a) }\).
    \end{cloze}
\end{note}

\begin{note}{9ab922ea4b1f422c855c9dc14925580a}
    Пусть \({ f : \langle A, B \rangle \to \mathbb R }\), \({ f }\) выпукла на \({ \langle A, B \rangle }\), \({ a \in (A, B) }\).
    Как показать, что прямая \( y = f(a) + k(x - a) \) является опорной для \({ f }\) \({ \impliedby }\) \({ k \in [f'_-(a), f'_+(a)] }\)?

    \begin{cloze}{1}
        Для \({ x \in (a, B \rangle }\) имеем \( F(x) (x - a) > f'_+(a) (x - a) \), где
        \[
            F(x) = \frac{f(x) - f(a)}{x - a}.
        \]

        Аналогично для \({ x \in \langle A, a) }\).
    \end{cloze}
\end{note}

\begin{note}{f8f5608de51344b89b749bf6fb673e89}
    Пусть \({ f : \langle A, B \rangle \to \mathbb R }\).
    Если в каждой точке \({ \langle A, B \rangle }\) функция \({ f }\) имеет опорную прямую, то \begin{icloze}{1}она выпукла на \({ \langle A, B \rangle }\).\end{icloze}
\end{note}

\begin{note}{0a5cbb4429524954af423e27fe0c32bc}
    Пусть \({ f : \langle A, B \rangle \to \mathbb R }\).
    Если в каждой точке \({ \langle A, B \rangle }\) функция \({ f }\) имеет строго опорную прямую, то \begin{icloze}{1}она строго выпукла на \({ \langle A, B \rangle }\).\end{icloze}
\end{note}
...
\begin{note}{2f438df6b5304cd7acbd273214875635}
    ...
    \begin{cloze}{1}
        От противного: допустить, что \({ \exists x_0 : f(x_0) > 0 }\) и использовать то, что \({ \exists \delta : f|_{V_{\delta}(x_0)} > 0 }\).
    \end{cloze}
\end{note}
...
\begin{note}{2255bf6318be43ee834d6071ed740c89}
    В чем основная идея доказательстве теоремы Барроу?

    \begin{cloze}{1}
        Используя непрерывность \({ f }\) представить \({ f(x) }\) как сумму \({ f(x_0) + \Delta x }\) и отсюда оценить \({ \varphi(x_0 + h) - \varphi(x_0) }\).
    \end{cloze}
\end{note}
...
\begin{note}{bddc676c2daa42f0b218c91eac244ace}
    ...
    \begin{cloze}{1}
        По теореме Лагранжа \({ \forall \tau \in T[a, b] }\) существует оснащение \({ \left\{ \xi_k \right\} }\) такое, что
        \[
            F(x_{k + 1}) - F(x_k) = F'(\xi_k) \Delta x_k.
        \]
    \end{cloze}
\end{note}
...
\begin{note}{1146663e8ded4d63ba85b1b1ed35cb14}
    Пусть \begin{icloze}{2}\({ f \in \mathcal R[a, b] }\),\: \({ F \in \mathscr P_f ([a, b] \setminus T) }\),\: \({ |T| < \aleph_0 }\).\end{icloze}
    Тогда
    \begin{icloze}{1}
        \[
            \int_{a}^{b} f = F(b) - F(a).
...
\begin{note}{496225bfd55a484fa6f61e7174af39f4}
    ...
    \begin{cloze}{1}
        Если \({ \left\{ a_i \right\}_{i = 1}^{n} }\)   --- все точки (a, b), в которых
    \end{cloze}
\end{note}
