...
\begin{note}{28934bf74ae1452191c8e81b8cef0cf5}
    Пусть
        ...
        \({ \left\{ e_i, f_j \right\} }\) --- базис в \({ V_1 }\),\:
        \({ \left\{ e_i, g_k \right\} }\) --- базис в \({ V_2 }\).
    Как показать, что \( \mathscr L (\left\{ g_k \right\}) \cap V_1 = \left\{  0 \right\} \)?

    \begin{cloze}{1}
        Если \({ \sum_{k}^{} \lambda_k g_k }\) принадлежит \({ \mathscr L (\left\{ g_k \right\}) \cap V_1 }\), то она  принадлежит и \({ V_1 \cap V_2 =  \mathscr L (\left\{ e_i \right\}) }\).
    \end{cloze}
\end{note}
